...
export const EndView = ({ scores, onRestart }: Props) => {
  const scoreCount = Object.entries(scores).reduce<Record<string, number>>((acc, [user, wins]) => {
    return {...acc, [user]: Object.values(wins).filter((isWin) => isWin === true).length}
  }, {})
  ...
  const numOfQuestions = Object.values(scores)?.[0] ? Object.keys(Object.values(scores)?.[0])?.length : 0
